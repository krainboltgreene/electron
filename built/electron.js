...
require.define("/lib/signal.coffee",function(require,module,exports,__dirname,__filename,process,global){(function() {
  var Event, Signal, exports, ff, _,
    __slice = [].slice;
  ...
  ff = function(f, args) {
    if (f) {
      return _.bind.apply(_, [f, {}].concat(__slice.call(args)));
    }
  };
  ...
  Signal = (function() {
    ...
    Signal.prototype.isDead = false;

    Signal.prototype.frameSize = 2;

    Signal.prototype.frame = [];

    Signal.prototype.linked = [];

    Signal.prototype.transforms = [];

    function Signal(transforms, frameSize) {
      this.transforms = transforms != null ? transforms : [
        (function(x) {
          return x;
        })
      ];
      this.frameSize = frameSize != null ? frameSize : 2;
      if (this.frameSize < 2) {
        this.frameSize = 2;
      }
    }

    Signal.prototype.setFrameSize = function(size) {
      this.frameSize = size;
      return this;
    };

    Signal.prototype.emit = function(value, meta, memoized) {
      var compute, computed, event,
        _this = this;
      if (this._events) {
        console.log(this);
      }
      ...
      if (meta && meta.isEnd) {
        ...
        this.links = [];
        this.transforms = [];
        _(this.linked).each(function(link) {
          if (this.linked) {
            return link.emit(value, meta, true);
          }
        });
        return false;
      }
      ...
      if (this.frame.length >= this.frameSize) {
        this.frame = __slice.call(_.rest(this.frame)).concat([event]);
      } else {
        this.frame = __slice.call(this.frame).concat([event]);
      }
      if (memoized) {
        computed = _(this.transforms).last()(event);
      } else {
        computed = (compute = function(i, evt) {
          var resultEvent;
          if (i < _this.transforms.length) {
            resultEvent = _this.transforms[i](evt);
          } else {
            return evt;
          }
          if (resultEvent && resultEvent.value) {
            return compute(i + 1, resultEvent);
          } else {
            return void 0;
          }
        })(0, event);
      ...
      _(this.linked).each(function(link) {
        if (computed) {
          return link.emit(computed.value, computed.meta, true);
        }
      });
      return this;
    };

    Signal.prototype.addTransform = function(transform) {
      var signal, transforms;
      transforms = __slice.call(this.transforms).concat([transform]);
      signal = new Signal(transforms, this.frameSize);
      this.linked = __slice.call(this.linked).concat([signal]);
      return signal;
    };

    Signal.prototype.fork = function() {
      var signal;
      signal = new Signal(this.transforms, this.frameSize);
      this.linked = __slice.call(this.linked).concat([signal]);
      return signal;
    };

    Signal.prototype.kill = function() {
      return this.emit("_commitsudoku", {
        isEnd: true
      });
    };

    Signal.prototype.react = function() {
      var args, f, _i;
      args = 2 <= arguments.length ? __slice.call(arguments, 0, _i = arguments.length - 1) : (_i = 0, []), f = arguments[_i++];
      return this.addTransform(function(event) {
        var result;
    ...
    Signal.prototype.span = function(size) {
      var _this = this;
      return this.addTransform(function(event) {
        var e;
        event.value = [
    ...
    Signal.prototype.log = function(logger) {
      if (logger == null) {
        logger = console.log;
      }
      return this.addTransform(function(event) {
    ...
    Signal.prototype.filter = function() {
      var args, f, _i;
      args = 2 <= arguments.length ? __slice.call(arguments, 0, _i = arguments.length - 1) : (_i = 0, []), f = arguments[_i++];
      return this.addTransform(function(event) {
        if (!ff(f, args)(event)) {
    ...
    Signal.prototype.skipDuplicates = function(isEqual) {
      var _this = this;
      if (isEqual == null) {
      ...
      return this.filter(function(event) {
        if (_.last(_.initial(_this.frame))) {
          return isEqual(_.last(_.initial(_this.frame)).value, event.value);
        } else if (event.value) {
          return false;
        } else {
          return true;
    ...
    Signal.prototype.merge = function(signal) {
      var _this = this;
      signal.react(function(event) {
        return _this.emit(event.value);
      });
      return this;
    };

    Signal.prototype.mergeInner = function(signal) {
      return this.merge(signal);
    };

    Signal.prototype.mergeOuter = function(signal) {
      this.react(function(event) {
        return signal.emit(event.value);
      });
      return this;
    };

    Signal.prototype.join = function(signal) {
      var newSignal,
        _this = this;
      newSignal = new Signal();
      signal.react(function(event) {
        return newSignal.emit([(_.last(_this.frame) ? _.last(_this.frame).value : void 0), event.value]);
      });
      return newSignal;
    };
...
require.define("/lib/event.coffee",function(require,module,exports,__dirname,__filename,process,global){(function() {
  var Event, exports;

  Event = (function() {

    Event.prototype.value = "default";

    Event.prototype.meta = {
      isError: false,
      isEnd: false,
      isNull: false
    };

    function Event(value, meta) {
...
require.define("/lib/bus.coffee",function(require,module,exports,__dirname,__filename,process,global){(function() {
  var Bus, Signal, exports, ff, _,
    __slice = [].slice;
  ...
  Bus = (function() {
    ...
    Bus.prototype.addSignal = function(signal) {
      this.signals.push(signal);
      return this;
    };

    Bus.prototype.addSignals = function() {
      var signals,
        _this = this;
      signals = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
      return _(signals).each(function(signal) {
        return _this.signals.push(signal);
      });
    };

    Bus.prototype.all = function() {
      var args, func, method, _i;
      method = arguments[0], args = 3 <= arguments.length ? __slice.call(arguments, 1, _i = arguments.length - 1) : (_i = 1, []), func = arguments[_i++];
      return _(this.signals).map(function(signal) {
        return signal[method].apply(signal, __slice.call(args).concat([func]));
      });
    };
    ...
    Bus.prototype.merge = function() {
      var newSignal;
      newSignal = new Signal();
      _(this.signals).each(function(signal) {
        return signal.react(function(event) {
          return newSignal.emit(event);
        });
      });
      return newSignal;
    };
...
require.define("/electron.coffee",function(require,module,exports,__dirname,__filename,process,global){(function() {
  var Bus, Electron, Event, Signal, exports, _, _ref,
    __slice = [].slice;
  ...
  Electron.fromEventTarget = function(target, eventName) {
    var handler, signal, unbind;
    signal = new Signal();
  ...
  Electron.fromPoll = function() {
    var args, func, signal, timer, _i;
    timer = arguments[0], args = 3 <= arguments.length ? __slice.call(arguments, 1, _i = arguments.length - 1) : (_i = 1, []), func = arguments[_i++];
    if (timer == null) {
      timer = 5000;
    }
    signal = new Signal();
    setInterval((function() {
      return signal.emit(func.apply(null, args));
    }), timer);
    return signal;
  };

  Electron.fromInterval = function(timer, value) {
    var signal;
    if (timer == null) {
      timer = 5000;
    }
  ...
  Electron.fromPromise = function(promise) {
    var signal;
    signal = new Signal();
  ...
  if (window) {
    ...
    if ((_ref = window.jQuery || window.Zepto) != null) {
      _ref.fn.asSignal = function(eventName) {
        var signal, unbind;
        signal = new Signal();
        unbind = function() {
          return this.off(eventName, (function(value) {
            return signal.emit(value);
          }));
        };
